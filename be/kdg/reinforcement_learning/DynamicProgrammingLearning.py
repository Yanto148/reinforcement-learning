...
from be.kdg.reinforcement_learning import Percept, MarkovDecisionProcess
    ...
    def improve(self, percept: Percept, t: int):
        for s in range(self.n_states):
            ...
            for a in range(len(self.policy[s])):
                if a == a_star:
                    self.policy[s, a] = 1 - self._epsilon + (self._epsilon / self.n_actions)
                else:
                    self.policy[s, a] = self._epsilon / self.n_actions
